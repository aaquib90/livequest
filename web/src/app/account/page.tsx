...
async function updateProfile(formData: FormData) {
  ...
  await supabase.auth.updateUser({
    data: { full_name: fullName || null, organisation: organisation || null, bio: bio || null },
  });
...
export default async function AccountPage({
  ...
  const liveblogRows = (liveblogs as AccountLiveblogRow[] | null) ?? [];
  const totalLiveblogs = liveblogRows.length;
  const activeLiveblogs = liveblogRows.filter(
    (lb) => lb.status === "active" || lb.status === null
  ).length;
  const archivedLiveblogs = liveblogRows.filter(
    (lb) => lb.status === "archived"
  ).length;
    ...
    <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-base text-muted-foreground">
                Manage your Livequest Studio identity, workspace preferences, and security controls in one place.
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-border/70 bg-background/50">
          <CardHeader>
            <CardTitle className="text-lg">Liveblogs published</CardTitle>
            <CardDescription>
              Active projects you&apos;re currently maintaining.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex items-baseline gap-3">
            ...
            <span className="text-sm text-muted-foreground">
              of {totalLiveblogs} total liveblogs
            </span>
          </CardContent>
        </Card>
        <Card className="border-border/70 bg-background/50">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <Card className="border-border/70 bg-background/50">
          <CardHeader>
            ...
            <CardDescription className="text-base">
              Update how your byline appears across liveblogs and newsletters. Changes apply immediately.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form action={updateProfile} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Publication, agency, or team"
                />
              </div>
              <div className="space-y-2">
        ...
        <Card className="border-border/70 bg-background/50">
          <CardHeader>
            ...
            <CardDescription className="text-base">
              Manage how you access Livequest Studio. Contact support for role-based access or SSO.
            </CardDescription>
          </CardHeader>
