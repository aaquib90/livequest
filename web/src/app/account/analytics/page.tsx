...
function Sparkline({
  ...
  const pathData = points
    ...
    .join(" ");
  const areaData = `${pathData} L ${points[points.length - 1].x.toFixed(
    2
  )} 100 L ${points[0].x.toFixed(2)} 100 Z`;
...
export default async function AccountAnalyticsPage() {
  ...
  const [summaryResult, timeseriesResult, topLiveblogsResult, topSponsorsResult, referrersResult] =
    await Promise.all([
      ...
      supabase.rpc("account_referrer_breakdown", { p_limit: 8 }),
    ]);
  ...
  const liveblogs = (topLiveblogsResult.data as TopLiveblogRow[] | null) ?? [];
  const sponsors = (topSponsorsResult.data as SponsorLeaderboardRow[] | null) ?? [];
  const referrers = (referrersResult.data as ReferrerRow[] | null) ?? [];

  const sponsorCtr =
    summary && summary.sponsor_ctr_30d !== null
      ? Number(summary.sponsor_ctr_30d)
      : 0;
  ...
  const timeseriesValues = timeseries.map((row) => row.unique_viewers ?? 0);
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card className="border-border/70 bg-background/50">
          <CardHeader>
            ...
            <CardDescription>Heartbeat pings that indicate dwell time and repeat visits.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <p className="text-4xl font-semibold text-foreground">
        ...
        <Card className="border-border/70 bg-background/50">
          <CardHeader className="space-y-4">
            ...
            <CardDescription className="text-base">
              Track how reach evolves each day to identify news cycles that resonate. Spikes can
              hint at traffic partners worth nurturing.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                ...
                <code className="font-mono text-xs">analytics_events</code> directly with SQL for
                bespoke dashboards.
              </p>
            </div>
            <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
